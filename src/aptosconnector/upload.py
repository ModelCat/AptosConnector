...
class DatasetUploader:
    ...
    def dataset_check(self):
        ...
        if self.ignore_validation:
            log.warning('Signature validation skipped (overriden by user). You dataset may not work on Aptos')
            return True
    ...
    def check_s3_access(self):
        ...
        if not self.is_valid_uuid(self.aptos_group_id):
            print('Provided `Aptos Group ID` does not have a correct format. It should be a valid UUID e.g. "461b1b66-8787-11ed-aff3-07f20767316e" ')

        cmd = ['aws', 's3',  'ls', f's3://aptos-data/account/{self.aptos_group_id}', '--profile', 'aptos_user']
        outputs = []
        try:
            run_cli_command(command=cmd, verbose=self.verbose, line_parser=lambda line: outputs.append(line.strip()))
    ...
    def upload_s3(self):
        ...
        num_files, size = self._count_files(self.dataset_root) #len([name for name in os.listdir('.') if os.path.isfile(name)])
        print(f'Found {num_files} files in the dataset: {self._convert_size(size)}')

        cmd = ['aws', 's3', 'sync', self.dataset_root, self.s3_uri, '--profile', 'aptos_user']
            
        print('')
        with tqdm(total=num_files, position=0) as pbar_top, tqdm(total=1, bar_format='{desc}', position=1) as pbar_bottom:
