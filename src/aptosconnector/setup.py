...
def run_setup(verbose: int = 0):
    ...
    if not check_awscli():
        ...
        print('For install instuctions go to: https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html')
        exit(1)
    else:
    ...
    # configure AWS CLI
    outputs = []
    append_fn = lambda l: outputs.append(l)
    try:
        cmd = ['aws', 'configure', 'set', 'region', _DEFAULT_REGION, '--profile', _DEFAULT_AWS_PROFILE]
        run_cli_command(cmd, line_parser=append_fn)
        cmd = ['aws', 'configure', 'set', 'format', _DEFAULT_FORMAT, '--profile', _DEFAULT_AWS_PROFILE]
        run_cli_command(cmd, line_parser=append_fn)
        cmd = ['aws', 'configure', 'set', 'aws_access_key_id', aws_access_key, '--profile', _DEFAULT_AWS_PROFILE]
        run_cli_command(cmd, line_parser=append_fn)
        cmd = ['aws', 'configure', 'set', 'aws_secret_access_key', aws_secret_access_key, '--profile', _DEFAULT_AWS_PROFILE]
        run_cli_command(cmd, line_parser=append_fn)
    ...
    print('\t`aptos_validate` to check your dataset for errors and verify Aptos interoperability')
